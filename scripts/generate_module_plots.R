...
dir.create("assets/module_plots", showWarnings = FALSE, recursive = TRUE)
...
umap_df <- read.csv("data/09_TOM_UMAP_embedding_leaf.csv", stringsAsFactors = FALSE)
...
for (mod in modules) {
  ...
  out_path <- file.path("assets", "module_plots", paste0("module_", mod, ".png"))
  ggsave(out_path, plot = p, width = 5, height = 3.2, dpi = 120)
  cat("  Saved:", out_path, "\n")
}

cat("Done! Generated", length(modules), "module plots in ./assets/module_plots/\n")
